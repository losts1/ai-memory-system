...
- grade 0: unrelated; or a superseded / removed fact when a newer active fact on the same topic is present in the list.
...
def parse_judgments(text: str, expected_names: Iterable[str]) -> Optional[Dict[str, dict]]:
    ...
    or None if the reply is not a well-formed grade array."""
...
def ndcg_at_k(ranked: List[str], grades: Dict[str, int], k: int) -> float:
    ...
    ideal = sorted((grades.get(n, 0) for n in ranked), reverse=True)[:k]
